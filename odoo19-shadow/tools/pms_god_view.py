...
def main():
    ...
    # 2. STAPS 2.0 Precision & Crypto Check
    ...
    if os.path.exists(staps_path):
        with open(staps_path, 'r') as f:
            ...
            print(f"  - MetricTensor Crypto  : {'ACTIVE' if has_crypto else 'MISSING'}")
    else:
    ...
    # 3.1 Fulfillment Telemetry & Security Audit
    ...
    if os.path.exists(ff_path):
        with open(ff_path, 'r') as f:
            ...
            has_obfuscation = "pms_id_secure" in content
            has_isolation = "resident_id" in content and "_handle_query" in content
            ...
            print(f"  - ID Obfuscation (V3)   : {'ENABLED' if has_obfuscation else 'MISSING'}")
            print(f"  - Resident Isolation    : {'VERIFIED' if has_isolation else 'FAILED'}")
    else:
    ...
    if os.path.exists(css_path):
        with open(css_path, 'r') as f:
            ...
            has_saturate = "saturate(180%)" in content
            print(f"  - Ultra-Intensity Blur (20px): {'ACTIVE' if has_blur_20 else 'FAILED'}")
            print(f"  - Glass Saturation (180%)    : {'OPTIMIZED' if has_saturate else 'FAILED'}")
