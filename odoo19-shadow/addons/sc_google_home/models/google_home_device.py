...
class GoogleHomeDevice(models.Model):
    ...
    traits = fields.Char(string='Traits (comma separated)', default='action.devices.traits.OnOff',
                         help="e.g. action.devices.traits.OnOff,action.devices.traits.Brightness,action.devices.traits.LockUnlock,action.devices.traits.SensorState")
    ...
    def _get_google_home_state(self):
        ...
        if 'action.devices.traits.SensorState' in trait_list:
            # Highest degree optimization: Community Score (Happiness Coins) reporting
            coins = self.partner_id.user_ids[:1].happiness_coin_balance if self.partner_id.user_ids else 0
