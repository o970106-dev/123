    ...
    def _handle_query(self, user, payload):
        ...
        for d in devices:
            ...
            if d.device_id:
                ...
                if d.device_type == 'lock':
                    # Assuming physical device has is_on mapped to locked/unlocked for simplicity or another field
                    state['isLocked'] = not d.device_id.is_on
            elif d.device_type == 'sensor':
                balance = user.partner_id.happiness_coin_balance if hasattr(user.partner_id, 'happiness_coin_balance') else 0
