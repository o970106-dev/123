...
class GoogleHomeFulfillment(http.Controller):
    ...
    def _validate_auth(self):
        ...
        # In a real authority implementation, we check the token against Odoo system parameters
        # or a dedicated OAuth2 provider.
    ...
    def fulfillment(self, **kwargs):
        ...
        if not self._validate_auth():
            # Return requestId if available to satisfy Google Home protocol even on auth failure
            req_data = {}
            try:
    ...
    def _handle_sync(self, request_id):
        # In a production OAuth2 flow, the agentUserId is the unique ID of the user
        # who linked their account. Here we use a unique hash based on the Odoo instance.
        agent_user_id = request.env['ir.config_parameter'].sudo().get_param('database.uuid') or 'pms-default-user'

        # Fetch devices. In optimized PMS, we could filter by user/room ownership.
