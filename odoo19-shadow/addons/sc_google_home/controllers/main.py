...
class GoogleHomeFulfillment(http.Controller):
    ...
    def _validate_auth(self):
        ...
        if not user:
            _logger.warning("Invalid Google Home Bearer Token")
            return False

        # Switch context to the authenticated user
    ...
    def _handle_sync(self, request_id):
        ...
        for dev in devices:
            ...
            if 'action.devices.traits.SensorState' in trait_list:
                dev_info['attributes']['sensorStates'] = [
                    {
            ...
            if 'action.devices.traits.OccupancySensing' in trait_list:
                # No specific attributes required for basic occupancy sensing
                pass
