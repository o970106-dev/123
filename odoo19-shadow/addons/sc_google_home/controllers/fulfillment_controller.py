...
class GoogleHomeFulfillment(http.Controller):
    ...
    def auth(self, **kwargs):
        """Mock OAuth2 Auth Endpoint - Now Requires Login."""
        redirect_uri = kwargs.get('redirect_uri')
        state = kwargs.get('state')
        # In a real scenario, this would redirect with an auth code linked to the current user
        return request.redirect(f"{redirect_uri}?code=supreme_auth_code&state={state}")
