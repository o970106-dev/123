...
class GoogleHomeFulfillment(http.Controller):
    ...
    def auth(self, **kwargs):
        ...
        code = user.google_home_authorization_code
    ...
    def token(self, **kwargs):
        ...
        # For POC purposes, we exchange the authorization code for the user's token.
        code = kwargs.get('code')
        user = request.env['res.users'].sudo().search([('google_home_authorization_code', '=', code)], limit=1)

        if not user:
            # Fallback for generic requests if needed
    ...
    def _handle_sync(self, user):
        ...
        for dev in devices:
            name_obj = {
                'name': dev.name,
                'nicknames': [n.strip() for n in dev.nicknames.split(',')] if dev.nicknames else [dev.name],
                'defaultNames': [d.strip() for d in dev.default_names.split(',')] if dev.default_names else [dev.name]
            }
            device_list.append({
