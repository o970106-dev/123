...
class GoogleHomeFulfillment(http.Controller):
    ...
    def fulfillment(self, **kwargs):
        ...
        if not user:
            res_data = {'requestId': payload.get('requestId'), 'payload': {'errorCode': 'authFailure'}}
    ...
    def _handle_query(self, user, payload):
        ...
        for gid in device_ids:
            ...
            if dev:
                ...
                elif dev.device_type == 'action.devices.types.SCENE':
                    status['online'] = True
                ...
                # Happiness Coins Balance Integration
                status['sensorStateData'] = [{
                    'name': 'Happiness Coins',
                    'numericValue': user.happiness_coin_balance
                }]
