...
class GoogleHomeFulfillment(http.Controller):
    ...
    def auth(self, **kwargs):
        ...
        # Generate secure authorization code for the current resident
        code = secrets.token_hex(16)
        request.env.user.sudo().google_home_authorization_code = code
        # Optimized to use request.redirect for professional account linking
        return request.redirect(f"{redirect_uri}?code={code}&state={state}")

    @http.route('/google_home/token', type='http', auth='public', methods=['POST'], csrf=False)
    def token(self, **kwargs):
        """Mock OAuth2 Token Endpoint."""
        # Validate account linking by exchanging authorization code for access token
        code = kwargs.get('code') or request.params.get('code')
        user = request.env['res.users'].sudo().search([('google_home_authorization_code', '=', code)], limit=1) if code else None

        if not user:
            # Fallback for demonstration if no code matches
            user = request.env['res.users'].sudo().search([('google_home_token', '!=', False)], limit=1)

        token = user.google_home_token if user else "mock_access_token_supreme"
    ...
    def _handle_sync(self, user):
        ...
        for dev in devices:
            device_list.append({
                ...
                'name': {
                    'name': dev.name,
                    'nicknames': [dev.name, dev.nicknames] if dev.nicknames else [dev.name],
                    'defaultNames': [dev.name, dev.default_names] if dev.default_names else [dev.name],
                },
