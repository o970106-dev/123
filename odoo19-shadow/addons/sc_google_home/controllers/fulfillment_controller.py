    ...
    def _handle_query(self, user, payload):
        ...
        for gid in device_ids:
            ...
            if dev:
                ...
                if dev.device_type == 'action.devices.types.LIGHT':
                    ...
                    status['thermostatMode'] = 'on' if dev.is_on else 'off'
            ...
            for execution in command.get('execution', []):
                ...
                for gid in device_ids:
                    ...
                    if dev:
                        if action == 'action.devices.commands.OnOff':
                        ...
                        elif action == 'action.devices.commands.ThermostatModes':
                            dev.is_on = (params.get('thermostatMode') != 'off')
                            ...
                            # Trigger Eco Mode optimization as a Scene
                            if 'eco' in dev.name.lower():
                                user.action_reward_sustainability(amount=10.0, reason="Eco Scene Activation")
