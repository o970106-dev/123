        ...
        for gid in device_ids:
            ...
            if dev:
                ...
                if dev.device_type == 'action.devices.types.LOCK':
                    ...
                    status['activeThermostatMode'] = 'heat' if dev.is_on else 'off'
                ...
                if 'action.devices.traits.Scene' in dev.get_google_traits():
                    # Scenes are typically stateless in QUERY, but we can report eco_mode
                    status['eco_mode'] = dev.eco_mode
