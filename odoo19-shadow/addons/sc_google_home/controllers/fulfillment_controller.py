                ...
                for gid in device_ids:
                    if gid == 'scene_eco_mode':
                        if action == 'action.devices.commands.ActivateScene':
                            # Trigger Eco Mode Optimization
                            user.action_reward_sustainability(amount=10.0, reason="Google Home Eco Scene Activation")
                            # Set all devices to energy saving state
                            user_devices = request.env['pms.device'].sudo().search([('resident_id', '=', user.id)])
                            for ud in user_devices:
                                if ud.device_type == 'action.devices.types.THERMOSTAT':
                                    ud.target_temperature = 26.0
                                ud.brightness = 50
