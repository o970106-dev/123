...
class GoogleHomeFulfillment(http.Controller):
    ...
    def _handle_sync(self, user):
        ...
        # Highest Degree Privacy: Obfuscate agentUserId
        agent_user_id = MetricTensorCryptoEngine.encode(user.id)
        return {'agentUserId': agent_user_id, 'devices': device_list}
    ...
    def _handle_query(self, user, payload):
        ...
        for gid in device_ids:
            ...
            if dev:
                ...
                elif dev.device_type == 'action.devices.types.THERMOSTAT' or dev.device_type == 'action.devices.types.SENSOR':
                ...
                if 'action.devices.traits.SensorState' in dev.get_google_traits():
                    status['currentSensorStateData'] = [
                        {
                            'name': 'occupancy',
                            'currentDescriptiveState': 'occupied' if dev.is_occupied else 'unoccupied'
                        }
                    ]
