...
class GoogleHomeFulfillment(http.Controller):
    ...
    def auth(self, **kwargs):
        ...
        if not redirect_uri.startswith('https://oauth-redirect.googleusercontent.com/'):
            _logger.warning(f"Invalid redirect_uri attempt: {redirect_uri}")
    ...
    def _handle_sync(self, user):
        ...
        # Supreme Security: Obfuscate agentUserId using MetricTensorCryptoEngine
    ...
    def _handle_query(self, user, payload):
        ...
        for gid in device_ids:
            # Supreme Security: Enforce ownership check ('resident_id', '=', user.id)
            ...
            if dev:
                ...
                elif dev.device_type in ['action.devices.types.THERMOSTAT', 'action.devices.types.SENSOR']:
                    status['thermostatTemperatureAmbient'] = dev.temperature
                    if dev.device_type == 'action.devices.types.THERMOSTAT':
                        status['thermostatTemperatureSetpoint'] = dev.target_temperature
                        status['thermostatMode'] = 'heat' if dev.is_on else 'off'

                    # Sensor specific fulfillment
                    if 'action.devices.traits.HumiditySetting' in dev.get_google_traits():
                        status['thermostatHumidityAmbient'] = dev.humidity

                    if 'action.devices.traits.OccupancySensing' in dev.get_google_traits():
                        status['currentSensorStateData'] = [{
                            'name': 'occupancy',
                            'currentDescriptiveState': dev.occupancy or 'UNKNOWN'
                        }]
