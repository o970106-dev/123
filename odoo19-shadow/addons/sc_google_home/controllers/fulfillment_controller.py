...
class GoogleHomeFulfillment(http.Controller):
    ...
    def _handle_sync(self, user):
        ...
        for dev in devices:
            device_list.append({
                ...
                'deviceInfo': {
                    'manufacturer': 'PMS Supreme IoT',
                    'model': 'PMS-OS-V3-Highest',
                    'hwVersion': '3.0.4-PRO',
                    'swVersion': '19.4.1-STAPS-ULTIMATE',
                },
                'customData': {
                    'pms_id': dev.id,
    ...
    def _handle_query(self, user, payload):
        ...
        for gid in device_ids:
            # Security: Verify that the device belongs to the authenticated resident
