...
class GoogleHomeFulfillment(http.Controller):
    ...
    def fulfillment(self, **kwargs):
        ...
        if not user:
            return request.make_response(
                json.dumps({'errorCode': 'authFailure'}),
    ...
    def _handle_sync(self, user):
        ...
        for dev in devices:
            ...
            if dev.device_type == 'action.devices.types.SCENE':
            ...
            if dev:
                if dev.device_type == 'action.devices.types.SCENE':
                    ...
                    status = {
                        ...
                        'currentFanSpeedSetting': 'High' if dev.fan_speed > 50 else 'Low',
                        'color': {'temperatureK': dev.color_temp},
            ...
            for execution in command.get('execution', []):
                ...
                for gid in device_ids:
                    ...
                    if dev:
                        if action == 'action.devices.commands.OnOff':
                        ...
                        elif action == 'action.devices.commands.ThermostatSetMode':
                            ...
                            dev.is_on = not params.get('deactivate', False)
