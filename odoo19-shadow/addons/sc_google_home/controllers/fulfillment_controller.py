...
class GoogleHomeFulfillment(http.Controller):
    ...
    def auth(self, **kwargs):
        ...
        # Following 'Highest Degree' standards for trusted Google OAuth prefixes
        allowed_prefixes = [
            'https://oauth-redirect.googleusercontent.com/',
            'https://oauth-redirect-sandbox.googleusercontent.com/'
        ]
        if not any(redirect_uri.startswith(prefix) for prefix in allowed_prefixes):
    ...
    def _handle_sync(self, user):
        ...
        for dev in devices:
            device_list.append({
                ...
                'customData': {
                    'pms_id': MetricTensorCryptoEngine.encode(dev.id),
