...
from odoo.addons.pms_base.models.staps_core import staps_timed, get_secure_coordinate
...
class GoogleHomeFulfillment(http.Controller):
    ...
    def auth(self, **kwargs):
        ...
        # Security: Validate redirect_uri against Google's official OAuth2 redirect domain
        allowed_domain = 'oauth-redirect.googleusercontent.com'
        if allowed_domain not in redirect_uri:
            return request.make_response("Invalid redirect_uri: Domain not authorized", status=400)
    ...
    def _handle_sync(self, user):
        ...
        for dev in devices:
            device_list.append({
                ...
                'customData': {
                    'pms_id': dev.id,
                    'pms_id_secure': get_secure_coordinate(dev.id),
