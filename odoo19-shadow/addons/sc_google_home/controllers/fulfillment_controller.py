...
class GoogleHomeFulfillment(http.Controller):
    ...
    def auth(self, **kwargs):
        ...
        # Security: Strict validation of redirect_uri for Google Home fulfillment
        if not redirect_uri.startswith('https://oauth-redirect.googleusercontent.com/'):
            return request.make_response("Invalid redirect_uri: Must be official Google OAuth redirect.", status=400)
    ...
    def token(self, **kwargs):
        """Professional OAuth2 Token Endpoint with Secure Refresh Token Logic."""
        ...
        if grant_type == 'authorization_code':
            ...
            if user:
                ...
                user.sudo().write({'google_home_authorization_code': False})
        elif grant_type == 'refresh_token':
            # Security: Validate the provided refresh token against the specific user
            if refresh_token:
                user = request.env['res.users'].sudo().search([('google_home_refresh_token', '=', refresh_token)], limit=1)

        if not user:
            return request.make_response(json.dumps({'error': 'invalid_grant'}), status=400, headers=[('Content-Type', 'application/json')])

        # Ensure user has persistent tokens
        user.sudo().action_generate_google_token()

        data = {
            "token_type": "bearer",
            "access_token": user.google_home_token,
            "refresh_token": user.google_home_refresh_token,
            "expires_in": 3600
        }
    ...
    def _handle_sync(self, user):
        ...
        # Privacy: Obfuscate agentUserId using MetricTensorCryptoEngine
        return {
            'agentUserId': MetricTensorCryptoEngine.encode(user.id),
            'devices': device_list
        }
    ...
    def _handle_query(self, user, payload):
        ...
        for gid in device_ids:
            # Security: Ensure device belongs to the user
            ...
            if dev:
                ...
                # Standard On/Off
                if 'action.devices.traits.OnOff' in dev.get_google_traits():
                    status['on'] = dev.is_on

                # Sensor Data Fulfillment
                if dev.device_type == 'action.devices.types.SENSOR':
                    status['thermostatTemperatureAmbient'] = dev.temperature
                    status['currentSensorStateData'] = [
                        {
                            'name': 'Occupancy',
                            'currentDescriptiveState': dev.occupancy,
                            'rawValue': dev.occupancy
                        },
                        {
                            'name': 'Humidity',
                ...
                # Lock Fulfillment
                elif dev.device_type == 'action.devices.types.LOCK':
                    status['isLocked'] = dev.is_locked
                    status['isJammed'] = False

                # Thermostat Fulfillment
