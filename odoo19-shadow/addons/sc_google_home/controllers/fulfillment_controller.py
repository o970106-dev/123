...
class GoogleHomeFulfillment(http.Controller):
    ...
    def fulfillment(self, **kwargs):
        ...
        try:
            ...
            return request.make_response(json.dumps({'errorCode': 'invalidRequest'}), headers=[('Content-Type', 'application/json')])
    ...
    def _handle_query(self, user, payload):
        ...
        for gid in device_ids:
            ...
            if dev:
                status = dev.get_google_state()
                # Global Happiness Coins Balance
