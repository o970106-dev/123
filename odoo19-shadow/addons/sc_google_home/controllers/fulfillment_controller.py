...
class GoogleHomeFulfillment(http.Controller):
    ...
    def auth(self, **kwargs):
        ...
        # Strict validation ensuring it starts with oauth-redirect.googleusercontent.com or is a trusted google.com domain
        allowed_prefixes = ('https://oauth-redirect.googleusercontent.com/', 'https://oauth-redirect-sandbox.googleusercontent.com/')
        if not any(redirect_uri.startswith(p) for p in allowed_prefixes) and not (redirect_uri.startswith('https://') and '.google.com' in redirect_uri):
    ...
    def _handle_sync(self, user):
        ...
        device_list = []
        for dev in devices:
            # Obfuscate internal ID for data sovereignty
            obfuscated_id = MetricTensorCryptoEngine.obfuscate_id(dev.id)
    ...
    def _handle_query(self, user, payload):
        ...
        for gid in device_ids:
            # Security: Ensure resident can only query their own devices
            dev = request.env['pms.device'].sudo().search([
                ('google_device_id', '=', gid),
                ('resident_id', '=', user.id)
            ], limit=1)
