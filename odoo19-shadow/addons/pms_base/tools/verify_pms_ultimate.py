...
def test_eco_score_logic():
    print("Testing Eco-Efficiency Score Logic (V3 Weighted)...")
    # Highest Degree Algorithm: 30% Coins, 30% Eco-Mode, 40% Variety
    ...
    eco_active = 4
    total = 4
    unique_types = 4 # variety_pts = 4 * 10 = 40

    coin_pts = min(30, coins * 0.6)
    eco_pts = (eco_active / total * 30)
    variety_pts = min(40, unique_types * 10)
    score = int(coin_pts + eco_pts + variety_pts)

    print(f"  - Perfect Score: {score} (Expected: 100)")
    assert score == 100

    # Case 2: Moderate Performance
    coins = 10 # 6 points
    devices = [
        MockPmsDevice({'device_type': 'action.devices.types.LIGHT', 'eco_mode': True}),
...
def test_metadata_fulfillment():
    print("\nTesting Metadata Fulfillment (Professional V3)...")
    ...
    assert sync_data['deviceInfo']['swVersion'] == '3.5.2'
