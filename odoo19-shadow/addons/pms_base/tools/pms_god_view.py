...
def main():
    ...
    if os.path.exists(pms_model_path):
        with open(pms_model_path, 'r') as f:
            content = f.read()
            traits = ['Brightness', 'Scene', 'TemperatureSetting', 'FanSpeed', 'SensorState']
            for trait in traits:
            ...
            print(f"  - SENSOR Type Support    : {'ACTIVE' if has_sensor else 'MISSING'}")
    else:
    ...
    if os.path.exists(ff_path):
        with open(ff_path, 'r') as f:
            ...
            has_id_sec = "MetricTensorCryptoEngine.encode(user.id)" in content
            has_uri_sec = "oauth-redirect.googleusercontent.com" in content
            has_sensor_data = "currentSensorStateData" in content
            print(f"  - Fulfillment Telemetry : {'ACTIVE' if has_telemetry else 'INACTIVE'}")
            print(f"  - agentUserId Obfuscate : {'ENABLED' if has_id_sec else 'DISABLED'}")
            print(f"  - Redirect URI Security : {'STRICT' if has_uri_sec else 'STANDARD'}")
            print(f"  - Sensor Data Handling  : {'SUPREME' if has_sensor_data else 'BASIC'}")
    else:
        print("  - Fulfillment controller not found!")

    # 4. Sustainability & Volunteer Nexus
    print("\n[4] Sustainability & Volunteer Nexus Audit:")
    coin_path = "odoo19-shadow/addons/pms_community_center/models/coin_models.py"
    portal_path = "odoo19-shadow/addons/pms_portal_resident/controllers/portal_controller.py"
    if os.path.exists(coin_path):
        with open(coin_path, 'r') as f:
            content = f.read()
            has_volunteer = "x_is_volunteer" in content
            has_skills = "pms.skill" in content
            print(f"  - Volunteer Framework   : {'INTEGRATED' if has_volunteer else 'MISSING'}")
            print(f"  - Skill Registry        : {'ACTIVE' if has_skills else 'INACTIVE'}")

    if os.path.exists(portal_path):
        with open(portal_path, 'r') as f:
            content = f.read()
            has_rate_limit = "timedelta(hours=1)" in content
            print(f"  - Reward Rate-Limiting  : {'ENFORCED' if has_rate_limit else 'DISABLED'}")

    # 5. UI/UX Glassmorphism & Heartbeat Check
    print("\n[5] Resident Portal UI/UX (Supreme Degree Glassmorphism):")
    ...
    if os.path.exists(css_path):
        with open(css_path, 'r') as f:
            ...
            print(f"  - Ultra-Intensity Blur  : {'ACTIVE (20px)' if has_blur_20 else 'FAILED'}")

    if os.path.exists(xml_path):
        with open(xml_path, 'r') as f:
            content = f.read()
            has_metric_tensor = "Metric Tensor:" in content and "ACTIVE" in content
            has_nexus = "Volunteer Nexus" in content
            print(f"  - Metric Tensor UI      : {'VISIBLE' if has_metric_tensor else 'MISSING'}")
            print(f"  - Volunteer Nexus UI    : {'ENABLED' if has_nexus else 'HIDDEN'}")
    else:
