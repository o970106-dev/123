...
def main():
    ...
    if os.path.exists(fulfillment_path):
        with open(fulfillment_path, 'r') as f:
            content = f.read()
            has_telemetry = "@staps_timed" in content
            has_nicknames = "nicknames" in content
            print(f"  - Fulfillment Telemetry  : {'ACTIVE' if has_telemetry else 'MISSING'}")
            print(f"  - Professional Metadata  : {'ENABLED' if has_nicknames else 'LEGACY'}")
    else:
