...
class PmsDevice(models.Model):
    ...
    occupancy = fields.Selection([
        ...
        ('UNKNOWN', 'Unknown'),
    ], string='Occupancy State', default='UNKNOWN')
    ...
    model_number = fields.Char(string='Model Number', default='PMS-v4-Supreme')
    hw_version = fields.Char(string='HW Version', default='4.0.0')
    sw_version = fields.Char(string='SW Version', default='4.1.0-STAPS-ULTIMATE')
    ...
    def get_google_attributes(self):
        ...
        if 'action.devices.traits.TemperatureSetting' in traits:
            attrs.update({
                ...
                'queryOnlyTemperatureSetting': True if self.device_type == 'action.devices.types.SENSOR' else False
            })
...
class PmsMaintenanceRequest(models.Model):
    ...
    device_id = fields.Many2one('pms.device', string='Related Device')
    ...
    def create(self, vals):
        ...
        if vals.get('name', 'NEW') == 'NEW':
            vals['name'] = f"REQ-{fields.Datetime.now().strftime('%Y%m%d%H%M%S')}"
        if not vals.get('staps_coordinate'):
            vals['staps_coordinate'] = get_staps_coordinate('maintenance_create')
        return super(PmsMaintenanceRequest, self).create(vals)
