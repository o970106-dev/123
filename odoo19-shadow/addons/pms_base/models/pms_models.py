...
class PmsDevice(models.Model):
    ...
    def get_google_attributes(self):
        ...
        if 'action.devices.traits.OccupancySensing' in traits:
            attrs.update({
                'occupancySensorRetentionTime': 300
            })
...
class PmsMaintenanceRequest(models.Model):
    ...
    _description = 'PMS Maintenance Request'
    _order = 'create_date desc'

    name = fields.Char(string='Request ID', required=True, default='NEW')
    ...
    def create(self, vals):
        ...
        if vals.get('name', 'NEW') == 'NEW':
            vals['name'] = f"REQ-{get_staps_coordinate('maintenance')[:8].upper()}"
