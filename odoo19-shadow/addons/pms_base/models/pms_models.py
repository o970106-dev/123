...
class PmsDevice(models.Model):
    ...
    def get_google_attributes(self):
        ...
        if 'action.devices.traits.SensorState' in self.get_google_traits():
            attrs.update({
                'sensorStates': [
                    {'name': 'Humidity', 'type': 'numeric', 'numericCapabilities': {'unit': 'percent'}},
                    {'name': 'Occupancy', 'type': 'descriptive', 'descriptiveCapabilities': {'availableStates': ['occupied', 'unoccupied', 'unknown']}}
                ]
            })
        return attrs
