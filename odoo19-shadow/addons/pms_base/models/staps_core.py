    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if persist:
                    try:
                        # Attempt to find environment and resident from args (self) or http request
                        env = None
                        resident_id = None
                        if args and hasattr(args[0], 'env'):
                            env = args[0].env
                            if hasattr(args[0], '_name') and args[0]._name == 'res.users':
                                resident_id = args[0].id
                            elif hasattr(args[0], 'resident_id'):
                                resident_id = args[0].resident_id.id
