    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # Persistent recording if requested
                if persist:
                    try:
                        # Highest Degree Reliability: Use a separate cursor to ensure persistence
                        # even if the main transaction is rolled back.
                        registry = http.request.env.registry if hasattr(http, 'request') and http.request else None
                        if not registry and args and hasattr(args[0], 'env'):
                            registry = args[0].env.registry

                        if registry:
                            with registry.cursor() as cr:
                                env = api.Environment(cr, 1, {}) # System user context
                        ...
                        _logger.error(f"Failed to persist telemetry (Independent Cursor): {record_error}")
        return wrapper
    return decorator
