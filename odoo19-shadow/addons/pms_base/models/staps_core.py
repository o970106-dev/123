...
class MetricTensorCryptoEngine:
    ...
    Uses a 2x2 Hill cipher for O(1) ID obfuscation.
    """
    MATRIX = [[3, 2], [5, 7]]
    ...
    def encode(cls, internal_id):
        """Encodes an integer ID into an 8-character hex coordinate."""
    ...
    def decode(cls, coordinate):
        """Decodes an 8-character hex coordinate back to an integer ID."""
        if not coordinate or len(coordinate) != 8:
            return None
        try:
            y1 = int(coordinate[:4], 16)
            y2 = int(coordinate[4:], 16)
