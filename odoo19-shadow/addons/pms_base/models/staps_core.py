        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if persist:
                    new_cr = None
                    try:
                        # Attempt to find registry from args (self) or http request
                        registry = None
                        ...
                        if registry:
                            # Use a separate cursor to ensure telemetry persistence even if the main transaction rolls back
                            new_cr = registry.cursor()
                            env = api.Environment(new_cr, 1, {}) # System user context

                            # Auto-capture resident_id from context
                            resident_id = False
                            if args and hasattr(args[0], 'id') and hasattr(args[0], '_name') and args[0]._name == 'res.users':
