...
class PmsPortal(http.Controller):
    ...
    def pms_dashboard(self, **kw):
        # Professional Account Linking: Auto-generate token if missing
        user = request.env.user
        if hasattr(user, 'action_generate_google_token'):
            user.sudo().action_generate_google_token()

        # High-Performance Pre-calculation for Dashboard Rendering
        devices = request.env['pms.device'].sudo().search([('resident_id', '=', user.id)])

        # Batch calculations to ensure sub-millisecond rendering
        vals = {
            'devices': devices,
            'coin_balance': user.happiness_coin_balance,
            'eco_score': user.eco_efficiency_score,
            'google_token': user.google_home_token,
            'optimization_badge': True
        }

        return request.render("pms_portal_resident.portal_pms_dashboard", vals)
