...
class PmsPortal(http.Controller):
    ...
    def pms_dashboard(self, **kw):
        ...
        # Pre-calculate values for "Highest Degree" performance rendering
        values = {
            'user': user,
            'devices': request.env['pms.device'].search([('resident_id', '=', user.id)]),
            'happiness_coin_balance': user.happiness_coin_balance if hasattr(user, 'happiness_coin_balance') else 0,
            'eco_efficiency_score': user.eco_efficiency_score if hasattr(user, 'eco_efficiency_score') else 0,
            'google_home_token': user.google_home_token if hasattr(user, 'google_home_token') else False,
        }
