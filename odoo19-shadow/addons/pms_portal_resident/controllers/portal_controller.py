...
class PmsPortal(http.Controller):
    ...
    def claim_reward(self):
        # Professional rate-limiting: Only one claim allowed per hour
        last_claim = request.env['pms.happiness.coin'].sudo().search([
            ('user_id', '=', request.env.user.id),
            ('reason', '=', 'Portal Sustainability Claim'),
            ('date', '>=', fields.Datetime.subtract(fields.Datetime.now(), hours=1))
        ], limit=1)

        if last_claim:
            return {'status': 'error', 'message': 'Sustainability reward already claimed this hour. Please wait.'}
