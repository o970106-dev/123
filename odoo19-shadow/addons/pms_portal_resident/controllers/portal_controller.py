...
class PmsPortal(http.Controller):
    ...
    def claim_reward(self):
        ...
        import datetime
        if last_reward and last_reward.date > (datetime.datetime.now() - datetime.timedelta(hours=1)):
             return {'status': 'error', 'message': 'Reward already claimed recently. Please wait.'}
    ...
    @http.route(['/pms/submit_maintenance'], type='json', auth="user")
    @staps_timed(persist=True)
    def submit_maintenance(self, description):
        if not description:
            return {'status': 'error', 'message': 'Description is required'}

        request.env['pms.maintenance.request'].sudo().create({
            'resident_id': request.env.user.id,
    ...
    def toggle_volunteer(self):
        ...
        return {'status': 'success', 'is_volunteer': user.x_is_volunteer}
