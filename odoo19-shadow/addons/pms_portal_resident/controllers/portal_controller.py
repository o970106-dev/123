...
class PmsPortal(http.Controller):
    ...
    def toggle_eco_mode(self):
        # Implementation of a global eco-mode for the resident
        request.env.user.action_reward_sustainability(amount=10.0, reason="Portal Eco-Mode Activation")
        # Logic to dim all lights for sustainability
        devices = request.env['pms.device'].search([('resident_id', '=', request.env.user.id), ('device_type', '=', 'action.devices.types.LIGHT')])
        for dev in devices:
            dev.brightness = 20
