...
class PmsPortal(http.Controller):
    ...
    @staps_timed(persist=True)
    def toggle_eco_mode(self, device_id=None):
        if device_id:
            device = request.env['pms.device'].browse(int(device_id))
            if device.resident_id.id == request.env.user.id:
                device.eco_mode = not device.eco_mode
                if device.eco_mode:
                    request.env.user.action_reward_sustainability(amount=1.0, reason="Eco-Mode Activated")
        else:
            # Global Toggle for user's devices
            devices = request.env['pms.device'].search([('resident_id', '=', request.env.user.id)])
            any_eco = any(d.eco_mode for d in devices)
            devices.write({'eco_mode': not any_eco})
            if not any_eco:
                request.env.user.action_reward_sustainability(amount=3.0, reason="Global Eco-Mode Activated")
