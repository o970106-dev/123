...
class PmsPortal(http.Controller):
    ...
    @staps_timed(persist=True)
    def toggle_volunteer(self, status):
        request.env.user.sudo().x_is_volunteer = status
        if status:
            request.env.user.action_reward_sustainability(amount=10.0, reason="Volunteer Onboarding Reward")
