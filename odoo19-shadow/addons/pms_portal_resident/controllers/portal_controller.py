...
class PmsPortal(http.Controller):
    ...
    @staps_timed(persist=True)
    def toggle_volunteer(self, volunteer_state):
        was_volunteer = request.env.user.x_is_volunteer
        request.env.user.x_is_volunteer = volunteer_state
        if volunteer_state and not was_volunteer:
            # Award 10 coins for first-time onboarding
            request.env.user.action_reward_sustainability(amount=10.0, reason="Volunteer Onboarding")
        return {'success': True, 'is_volunteer': request.env.user.x_is_volunteer}
    ...
    def update_skills(self, skill_id):
        ...
        if skill.exists():
            request.env.user.x_volunteer_skills = [(4, skill.id)]
            return {'success': True}
