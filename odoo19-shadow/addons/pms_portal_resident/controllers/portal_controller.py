...
class PmsPortal(http.Controller):
    ...
    def pms_dashboard(self, **kw):
        ...
        # Prepare context for V4 dashboard
        devices = request.env['pms.device'].sudo().search([('resident_id', '=', request.env.user.id)])
        maintenance = request.env['pms.maintenance.request'].sudo().search([('resident_id', '=', request.env.user.id)], order='date_submitted desc', limit=5)
        skills = request.env['pms.skill'].sudo().search([])
    ...
    def toggle_volunteer(self):
        ...
        return {'success': True, 'is_volunteer': user.x_is_volunteer}
    ...
    def claim_reward(self):
        ...
        # 1-hour rate limiting check
        last_claim = request.env['pms.happiness.coin'].sudo().search([
            ('user_id', '=', user.id),
            ('reason', '=', 'Portal Sustainability Claim'),
            ('date', '>', fields.Datetime.now() - timedelta(hours=1))
        ], limit=1)

        if last_claim:
            return {'success': False, 'message': 'Rate limited. Please wait 1 hour.'}

        user.action_reward_sustainability(amount=5.0, reason="Portal Sustainability Claim")
        return {'success': True, 'new_balance': user.happiness_coin_balance}
