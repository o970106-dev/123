...
class PmsPortal(http.Controller):
    ...
    def toggle_volunteer(self):
        ...
        new_state = not user.x_is_volunteer
        user.sudo().write({'x_is_volunteer': new_state})

        if new_state:
            # Grant a one-time onboarding reward for becoming a volunteer
            user.action_reward_sustainability(amount=10.0, reason="Volunteer Onboarding")

        return {'success': True, 'is_volunteer': new_state, 'new_balance': user.happiness_coin_balance}
