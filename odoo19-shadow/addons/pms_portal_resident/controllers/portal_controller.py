...
class PmsPortal(http.Controller):
    ...
    def toggle_device(self, device_id):
        ...
        if device.resident_id.id != request.env.user.id:
            return {'success': False, 'status': 'error', 'message': 'Access Denied'}

        device.is_on = not device.is_on
        return {'success': True, 'status': 'success', 'new_state': device.is_on}
    ...
    def claim_reward(self):
        ...
        return {'success': True, 'status': 'success', 'new_balance': request.env.user.happiness_coin_balance}
