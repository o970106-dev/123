...
class PmsPortal(http.Controller):
    ...
    def pms_dashboard(self, **kw):
        ...
        coins = request.env['pms.happiness.coin'].sudo().search([('resident_id', '=', user.id)], limit=5, order='id desc')
        vals = {
            'user_id': user,
            'devices': devices,
            'coins': coins,
            'happiness_coin_balance': user.happiness_coin_balance,
        }
    ...
    def toggle_eco_mode(self, device_id):
        ...
        # Optimization: When Eco Mode is on, reduce brightness to 30%
        if device.eco_mode:
            device.brightness = 30
        return {'status': 'success', 'eco_mode': device.eco_mode, 'brightness': device.brightness}
