...
class PmsPortal(http.Controller):
    ...
    @http.route(['/pms/device/toggle'], type='json', auth="user")
    ...
    @http.route(['/pms/device/brightness'], type='json', auth="user")
    @staps_timed(persist=True)
    ...
    @http.route(['/pms/device/fan_speed'], type='json', auth="user")
    @staps_timed(persist=True)
    ...
    @http.route(['/pms/device/color_temp'], type='json', auth="user")
    @staps_timed(persist=True)
    ...
    @http.route(['/pms/device/eco'], type='json', auth="user")
    @staps_timed(persist=True)
    ...
    def claim_reward(self):
        # 1-hour rate-limiting for sustainability reward claims
        ...
        if last_claim:
            return {'success': False, 'message': 'You can only claim this reward once per hour.'}
