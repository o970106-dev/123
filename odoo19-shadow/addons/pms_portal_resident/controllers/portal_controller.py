...
class PmsPortal(http.Controller):
    ...
    def pms_dashboard(self, **kw):
        ...
        # High-performance dashboard data pre-calculation
        # Optimization: Fetching devices and coins in batch to reduce SQL overhead
        devices = request.env['pms.device'].search([('resident_id', '=', user.id)])
        happiness_coin_balance = user.happiness_coin_balance
        eco_efficiency_score = user.eco_efficiency_score

        # Prepare rendering context
        values = {
            'user': user,
            'devices': devices,
            'happiness_coin_balance': happiness_coin_balance,
            'eco_efficiency_score': eco_efficiency_score,
            'google_home_token': user.google_home_token,
        }
