...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Volunteer Status Toggle Logic
    const volunteerToggle = document.getElementById('volunteer_status_toggle');
    if (volunteerToggle) {
        volunteerToggle.addEventListener('change', async (ev) => {
            const res = await rpc("/pms/toggle_volunteer", {});
            if (res.status === 'success') {
                const label = document.getElementById('volunteer_status_label');
                if (label) {
                    label.innerText = res.is_volunteer ? 'ACTIVE' : 'INACTIVE';
                    label.className = res.is_volunteer ? 'text-success' : 'text-muted';
                }
                // Refresh balance if onboarding reward granted
                if (res.is_volunteer) {
                    rpc("/pms/claim_reward", {}).then(r => {
                        const balanceDisplay = document.querySelector('.coin-balance-display .h1');
                        if (balanceDisplay) balanceDisplay.innerText = r.new_balance;
                    });
                }
            }
        });
    ...
    // Add Skill Logic
    const addSkillBtn = document.getElementById('btn_add_skill');
    if (addSkillBtn) {
        addSkillBtn.addEventListener('click', async () => {
            const skillId = document.getElementById('available_skills_select').value;
            if (!skillId) return;
            const res = await rpc("/pms/add_skill", { skill_id: skillId });
            if (res.status === 'success') {
