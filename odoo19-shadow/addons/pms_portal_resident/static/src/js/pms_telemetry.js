...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (volunteerToggle) {
        volunteerToggle.addEventListener('change', async (ev) => {
            const res = await rpc("/pms/toggle_volunteer", { volunteer_state: ev.target.checked });
            if (res.success) {
                // Refresh if needed or update UI status
            }
        });
    }
    ...
    if (skillSelector) {
        skillSelector.addEventListener('change', async (ev) => {
            ...
            if (skillId) {
                ...
                if (res.success) {
                    location.reload(); // Reload to show new skill badge
                }
            }
        });
    }
