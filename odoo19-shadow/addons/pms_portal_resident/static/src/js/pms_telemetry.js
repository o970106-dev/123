...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateTelemetry = () => {
        ...
        rpc("/pms/staps_ping", {}).then((res) => {
            ...
            if (coordEl && res && res.success && res.coordinate) {
    ...
    document.querySelectorAll('.pms-fan-speed').forEach(slider => {
        slider.addEventListener('change', async (ev) => {
            ...
            await rpc("/pms/set_fan_speed", { device_id: device_id, fan_speed: fanSpeed });
        });
    });
    ...
    document.querySelectorAll('.pms-color-temp').forEach(slider => {
        slider.addEventListener('change', async (ev) => {
            ...
            await rpc("/pms/set_color_temp", { device_id: device_id, color_temp: colorTemp });
        });
    });
    ...
    document.querySelectorAll('.pms-eco-toggle').forEach(toggle => {
        toggle.addEventListener('change', async (ev) => {
            ...
            await rpc("/pms/toggle_eco_mode", { device_id: device_id, eco_state: ecoState });
        });
    });
    ...
    if (claimBtn) {
        claimBtn.addEventListener('click', async () => {
            ...
            if (res.success) {
                const balanceDisplay = document.querySelector('.display-4.text-warning');
                if (balanceDisplay) balanceDisplay.innerText = res.new_balance;
                alert("Sustainability reward claimed successfully!");
            } else if (res.message) {
                alert(res.message);
            }
        });
    ...
    const volToggle = document.getElementById('volunteer_toggle');
    if (volToggle) {
        volToggle.addEventListener('change', async (ev) => {
