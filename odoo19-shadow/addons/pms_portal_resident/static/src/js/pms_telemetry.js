...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.pms-toggle').forEach(btn => {
        btn.addEventListener('click', async (ev) => {
            ...
            const res = await rpc("/pms/device/toggle", { device_id: deviceId });
            if (res.success) {
    ...
    document.querySelectorAll('.pms-brightness').forEach(slider => {
        slider.addEventListener('change', async (ev) => {
            ...
            await rpc("/pms/device/brightness", { device_id: deviceId, brightness: brightness });
        });
    });
    ...
    document.querySelectorAll('.pms-fan-speed').forEach(slider => {
        slider.addEventListener('change', async (ev) => {
            ...
            await rpc("/pms/device/fan_speed", { device_id: deviceId, fan_speed: fanSpeed });
        });
    });
    ...
    document.querySelectorAll('.pms-color-temp').forEach(slider => {
        slider.addEventListener('change', async (ev) => {
            ...
            await rpc("/pms/device/color_temp", { device_id: deviceId, color_temp: colorTemp });
        });
    });
    ...
    document.querySelectorAll('.pms-eco-toggle').forEach(toggle => {
        toggle.addEventListener('change', async (ev) => {
            ...
            await rpc("/pms/device/eco", { device_id: deviceId, eco_state: ecoState });
        });
    });
    ...
    // Volunteer Toggle Logic
    const volunteerToggle = document.getElementById('pms_volunteer_toggle');
    if (volunteerToggle) {
        volunteerToggle.addEventListener('change', async () => {
            const res = await rpc("/pms/toggle_volunteer", {});
            if (res.success) {
                location.reload(); // Refresh to show/hide skills or onboarding reward
            }
        });
    }
});
