...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.pms-brightness').forEach(slider => {
        slider.addEventListener('change', async (ev) => {
            ...
            if (res.status === 'success') {
                ev.target.parentElement.querySelector('.brightness-val').innerText = res.brightness;
            }
        });
    });
    ...
    if (ecoBtn) {
        ecoBtn.addEventListener('click', async () => {
            ...
            if (res.status === 'success') {
                alert(`Sustainability Mode Active! New Balance: ${res.new_balance} Coins`);
                location.reload(); // Reload to reflect dimmed lights
            }
        });
    }
});
