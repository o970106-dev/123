...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (addSkillBtn) {
        addSkillBtn.addEventListener('click', async () => {
            ...
            const skillId = skillSelector.value;
            if (!skillId) return;

            const res = await rpc("/pms/update_skills", { skill_id: skillId });
            if (res.success) {
                const skillList = document.getElementById('pms_skill_list');
                if (skillList) {
                    skillList.innerHTML = res.skills.map(s => `<span class="badge badge-info mr-1 mb-1">${s.name}</span>`).join('');
                }
                // Remove the added skill from the selector
                skillSelector.querySelector(`option[value="${skillId}"]`).remove();
                skillSelector.value = "";
            }
        });
    }
