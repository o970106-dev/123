...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Volunteer Toggle Interactive Logic
    ...
    if (volunteerToggle) {
        volunteerToggle.addEventListener('change', async (ev) => {
            ...
            if (res.success) {
                // Update Happiness Coin balance on dashboard
                const balanceDisplay = document.querySelector('.coin-balance-display .h1');
                if (balanceDisplay && res.new_balance !== undefined) {
                    balanceDisplay.innerText = res.new_balance;

                    // Visual feedback for reward
                    if (res.is_volunteer) {
                        balanceDisplay.style.transition = 'transform 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275)';
                        balanceDisplay.style.transform = 'scale(1.3)';
                        setTimeout(() => balanceDisplay.style.transform = 'scale(1)', 600);
                    }
                }
            } else {
