...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.pms-brightness').forEach(input => {
        input.addEventListener('change', async (ev) => {
            const deviceId = ev.target.dataset.id;
            const brightness = ev.target.value;
            const res = await rpc("/pms/set_brightness", { device_id: deviceId, brightness: brightness });
            if (res.status === 'success') {
                const brightnessSpan = ev.target.closest('tr').querySelector('td span.badge').parentElement.querySelector('small span');
                if (brightnessSpan) brightnessSpan.innerText = res.new_brightness;
                console.log(`Device ${deviceId} brightness set to ${res.new_brightness}%`);
            }
        });
    });
    ...
    // Eco Mode Logic
    const ecoBtn = document.getElementById('btn_eco_mode');
    if (ecoBtn) {
        ecoBtn.addEventListener('click', async () => {
            // Mock triggering Eco Mode via JS
            alert("Triggering Eco Mode optimization across all devices...");
            const res = await rpc("/pms/claim_reward", { reason: "Eco Mode Portal Trigger" });
            if (res.status === 'success') {
                const balanceDisplay = document.querySelector('.coin-balance-display .h1');
                if (balanceDisplay) balanceDisplay.innerText = res.new_balance;
                location.reload(); // Refresh to show updated device states
            }
        });
    }
});
