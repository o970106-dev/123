...
document.addEventListener('DOMContentLoaded', () => {
        ...
        btn.addEventListener('click', async (ev) => {
            ...
            if (res.status === 'success') {
                ...
                if (res.new_state) {
                    ...
                    statusBadge.className = 'badge badge-secondary mr-3';
    ...
    document.querySelectorAll('.pms-brightness').forEach(slider => {
        slider.addEventListener('change', async (ev) => {
            ...
            if (res.status === 'success') {
                const valDisplay = ev.target.nextElementSibling;
                if (valDisplay) valDisplay.innerText = `${res.brightness}%`;
                console.log(`Device ${deviceId} brightness set to ${res.brightness}`);
            }
        });
    });

    // Eco Mode Toggle Logic
    const ecoBtn = document.getElementById('btn_toggle_eco');
    if (ecoBtn) {
        ecoBtn.addEventListener('click', async () => {
            const res = await rpc("/pms/toggle_eco_mode", {});
            if (res.status === 'success') {
                if (res.eco_mode) {
                    ecoBtn.className = 'btn btn-lg btn-success btn-block';
                    ecoBtn.innerHTML = '<i class="fa fa-leaf mr-2"/> Eco Mode: ON';
                    alert('Eco Mode Activated! 5 Happiness Coins earned.');
                } else {
                    ecoBtn.className = 'btn btn-lg btn-outline-success btn-block';
                    ecoBtn.innerHTML = '<i class="fa fa-leaf mr-2"/> Activate Eco Mode';
                }
                // Refresh balance display
                rpc("/my/pms", {}).then(() => window.location.reload());
            } else {
                alert(res.message);
            }
        });
