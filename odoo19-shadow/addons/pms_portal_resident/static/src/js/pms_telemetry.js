...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.pms-toggle').forEach(btn => {
        btn.addEventListener('click', async (ev) => {
            ...
            if (res.status === 'success') {
                ...
                if (res.new_state) {
                    statusBadge.className = 'badge badge-success px-3';
                    statusBadge.innerText = 'ACTIVE';
                } else {
                    statusBadge.className = 'badge badge-secondary px-3';
    ...
    document.querySelectorAll('.pms-brightness').forEach(slider => {
        slider.addEventListener('change', async (ev) => {
            ...
            if (res.status === 'success') {
                console.log(`Brightness for ${deviceId} set to ${res.brightness}`);
            }
        });
    });

    // Global Eco Toggle
    const ecoGlobalBtn = document.getElementById('btn_toggle_eco');
    if (ecoGlobalBtn) {
        ecoGlobalBtn.addEventListener('click', async () => {
            const res = await rpc("/pms/toggle_eco_mode", {});
            if (res.status === 'success') {
                location.reload(); // Quick refresh to update all eco badges
            }
        });
    }
    ...
    if (claimBtn) {
        claimBtn.addEventListener('click', async () => {
            ...
            if (res.status === 'success') {
                ...
                if (balanceDisplay) balanceDisplay.innerText = res.new_balance.toFixed(2);
            }
        });
    }
});
