...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateTelemetry = () => {
        ...
        rpc("/pms/staps_ping", {}).then((res) => {
            ...
            if (latencyEl) {
                ...
                if (duration < 50) latencyEl.classList.add('text-success');
                else if (duration < 150) latencyEl.classList.add('text-warning');
                else latencyEl.classList.add('text-danger');
            }
    ...
    document.querySelectorAll('.pms-color-temp').forEach(slider => {
        slider.addEventListener('change', async (ev) => {
            ...
            // Note: This endpoint should be added to the controller for full optimization
            await rpc("/pms/set_color_temp", { device_id: deviceId, color_temp: colorTemp });
        });
    });

    // Fan Speed Logic
    document.querySelectorAll('.pms-fan-speed').forEach(select => {
