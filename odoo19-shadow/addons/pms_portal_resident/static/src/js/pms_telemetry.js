...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const volunteerDetails = document.getElementById('volunteer_details');
    if (volunteerToggle) {
        volunteerToggle.addEventListener('change', async (ev) => {
            ...
            if (res.status === 'success') {
                if (active) {
                    volunteerDetails.classList.add('show');
                } else {
                    volunteerDetails.classList.remove('show');
                }
            }
        });
    }

    const saveSkillsBtn = document.getElementById('btn_save_skills');
    if (saveSkillsBtn) {
        saveSkillsBtn.addEventListener('click', async () => {
            const skillIds = Array.from(document.querySelectorAll('.skill-checkbox:checked')).map(cb => parseInt(cb.dataset.id));
            const res = await rpc("/pms/update_skills", { skill_ids: skillIds });
            if (res.status === 'success') {
                alert('Professional Profile Updated at Highest Degree.');
            }
        });
    }
});
