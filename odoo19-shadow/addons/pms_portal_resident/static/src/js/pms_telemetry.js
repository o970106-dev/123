...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (volunteerToggle) {
        volunteerToggle.addEventListener('change', async (ev) => {
            ...
            if (res.status === 'success') {
                const statusLabel = document.getElementById('volunteer_status_label');
                if (statusLabel) {
                    statusLabel.innerText = res.is_volunteer ? 'Active Volunteer' : 'Inactive';
                    statusLabel.className = res.is_volunteer ? 'text-success font-weight-bold' : 'text-muted';
                }
            }
        });
    }
