...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateTelemetry = () => {
        ...
        rpc("/pms/staps_ping", {}).then((res) => {
            ...
            // Supreme Heartbeat Optimization
            if (heartbeatEl) {
                heartbeatWidth = (heartbeatWidth + 25) % 125;
                const displayWidth = heartbeatWidth > 100 ? 100 : heartbeatWidth;
                heartbeatEl.style.width = displayWidth + '%';
                heartbeatEl.setAttribute('aria-valuenow', displayWidth);
            }
        }).catch(() => {
             const duration = (performance.now() - startTime).toFixed(4);
