...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateTelemetry = () => {
        ...
        jsonrpc("/pms/staps_ping", {}).then((res) => {
    ...
    document.querySelectorAll('.pms-toggle').forEach(btn => {
        btn.addEventListener('click', async (ev) => {
            ...
            const res = await jsonrpc("/pms/toggle_device", { device_id: deviceId });
    ...
    document.querySelectorAll('.pms-brightness').forEach(slider => {
        slider.addEventListener('change', async (ev) => {
            ...
            await jsonrpc("/pms/set_brightness", { device_id: deviceId, brightness: brightness });
        });
    });
    ...
    document.querySelectorAll('.pms-fan-speed').forEach(slider => {
        slider.addEventListener('change', async (ev) => {
            ...
            await jsonrpc("/pms/set_fan_speed", { device_id: deviceId, fan_speed: fanSpeed });
        });
    });
    ...
    document.querySelectorAll('.pms-color-temp').forEach(slider => {
        slider.addEventListener('change', async (ev) => {
            ...
            await jsonrpc("/pms/set_color_temp", { device_id: deviceId, color_temp: colorTemp });
        });
    });
    ...
    document.querySelectorAll('.pms-eco-toggle').forEach(toggle => {
        toggle.addEventListener('change', async (ev) => {
            ...
            await jsonrpc("/pms/toggle_eco_mode", { device_id: deviceId, eco_state: ecoState });
        });
    });
