...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (volToggle) {
        volToggle.addEventListener('change', async (ev) => {
            ...
            if (res.success) {
                // Refresh to update score/coins if needed, or just update UI
    ...
    if (skillSelector) {
        skillSelector.addEventListener('change', async (ev) => {
            const skillId = parseInt(ev.target.value);
            if (skillId) {
                // In a real multi-select we'd get all, here we just add one for demo
                const res = await rpc("/pms/update_skills", { skill_ids: [skillId] });
