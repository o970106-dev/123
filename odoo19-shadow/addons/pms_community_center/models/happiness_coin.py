...
class ResUsers(models.Model):
    ...
    def action_reward_sustainability(self):
        """
        Highest degree optimization: Automatically award coins for eco-friendly behavior.
        Called by PF node when resident activates Energy Saving Mode.
        """
        for user in self:
            self.env['pms.happiness.coin'].sudo().create({
                'resident_id': user.id,
                'amount': 5.0,
                'transaction_type': 'earn',
                'description': 'Sustainability Reward: Energy Saving Mode Active'
            })
        return True
