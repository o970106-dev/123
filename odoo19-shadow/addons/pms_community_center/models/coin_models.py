...
class ResUsers(models.Model):
    ...
    def _compute_eco_score(self):
        ...
        maint_requests = self.env['pms.maintenance.request'].sudo().search([('resident_id', 'in', self.ids)])
        maint_map = {}
        ...
        for user in self:
            ...
            vol_points = 15 if user.x_is_volunteer else 0

            # 5. Maintenance Weight (Max 15 points)
            user_maint = maint_map.get(user.id, [])
            maint_points = min(15, len(user_maint) * 3)

            user.eco_efficiency_score = int(min(100, coin_points + eco_points + variety_points + vol_points + maint_points))
