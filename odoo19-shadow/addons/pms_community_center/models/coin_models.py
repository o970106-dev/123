...
class ResUsers(models.Model):
    ...
    def _compute_eco_score(self):
        for user in self:
            # Score based on energy-saving devices and coin balance
            eco_devices = user.pms_device_ids.filtered(lambda d: d.eco_mode or not d.is_on)
            device_score = (len(eco_devices) / len(user.pms_device_ids)) * 100 if user.pms_device_ids else 50
            user.eco_efficiency_score = (device_score + min(user.happiness_coin_balance, 100)) / 2.0
