...
class PmsSkill(models.Model):
    _name = 'pms.skill'
    _description = 'Resident Volunteer Skill'
...
class ResUsers(models.Model):
    ...
    required_skill_id = fields.Many2one('pms.skill', string='Required Skill for Task')
    ...
    def _compute_eco_score(self):
        ...
        - 15% Volunteering Activity (Community Engagement)
        - 15% Maintenance Participation (Infrastructure Care)
        """
        # Batch search all devices and maintenance requests for the requested users to avoid O(N^2)
        ...
        maint_res = self.env['pms.maintenance.request'].read_group(
            [('resident_id', 'in', self.ids)],
            ['resident_id'],
            ['resident_id']
        )
        maint_map = {item['resident_id'][0]: item['resident_id_count'] for item in maint_res}
        ...
        for user in self:
            ...
            maint_reqs = maint_map.get(user.id, 0)
            maint_points = min(15, maint_reqs * 5)

            user.eco_efficiency_score = int(min(100, coin_points + eco_points + variety_points + volunteer_points + maint_points))

    def get_obfuscated_id(self):
        """Returns Metric Tensor obfuscated ID for secure frontend use."""
        from odoo.addons.pms_base.models.staps_core import MetricTensorCryptoEngine
        self.ensure_one()
        return MetricTensorCryptoEngine.encode(self.id)
