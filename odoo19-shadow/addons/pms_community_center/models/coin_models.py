...
class ResUsers(models.Model):
    ...
    def _compute_eco_score(self):
        ...
        - 15% Maintenance (Property responsibility)
        """
        # Batch search all devices for the requested users
        ...
        all_maint = self.env['pms.maintenance.request'].sudo().search([('resident_id', 'in', self.ids)])
        maint_map = {}
        for req in all_maint:
            maint_map.setdefault(req.resident_id.id, []).append(req)
        ...
        for user in self:
            ...
            volunteer_points = 15 if user.x_is_volunteer and user.x_volunteer_skills else 0

            # 5. Maintenance Weight (Max 15 points)
            user_maint = maint_map.get(user.id, [])
            resolved_maint = len([r for r in user_maint if r.status == 'resolved'])
            maint_points = min(15, resolved_maint * 5)
