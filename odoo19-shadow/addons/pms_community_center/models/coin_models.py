...
class ResUsers(models.Model):
    ...
    def _compute_eco_score(self):
        ...
        for user in self:
            # 1. Happiness Coin Weight (Max 30 points): (min(30, coins * 0.6))
            coin_points = min(30.0, user.happiness_coin_balance * 0.6)

            # 2. Eco-Mode Weight (Max 30 points): ((eco_active / total) * 30)
            user_devices = device_map.get(user.id, [])
            total_dev = len(user_devices)
            eco_dev = len([d for d in user_devices if d.eco_mode])
            eco_points = (eco_dev / total_dev * 30.0) if total_dev > 0 else 0.0

            # 3. Variety Weight (Max 40 points): (min(40, variety * 10))
            unique_types = len(set(d.device_type for d in user_devices))
            variety_points = min(40.0, unique_types * 10.0)

            user.eco_efficiency_score = int(round(min(100.0, coin_points + eco_points + variety_points)))
