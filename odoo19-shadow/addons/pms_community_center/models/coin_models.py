...
class PmsSkill(models.Model):
    _name = 'pms.skill'
    _description = 'Resident Volunteering Skill'
...
class ResUsers(models.Model):
    ...
    required_skill_id = fields.Many2one('pms.skill', string='Primary Skill')
    ...
    def _compute_eco_score(self):
        ...
        """
        # Batch search all required records to avoid O(N^2)
        ...
        all_maintenance = self.env['pms.maintenance.request'].sudo().search([('resident_id', 'in', self.ids)])
        maint_map = {}
        ...
        for user in self:
            ...
            maint_points = min(15, len(user_maint) * 5)
