...
def main():
    ...
    if os.path.exists(fulfillment_path) and os.path.exists(model_path):
        ...
        traits = [
            ('OnOff', f_content),
            ('LockUnlock', f_content),
            ('TemperatureSetting', m_content),
            ('FanSpeed', f_content),
            ('ColorTemperature', m_content),
            ('SensorState', f_content),
            ('Brightness', m_content),
            ('Scene', f_content),
            ('roomHint', f_content)
        ]
        for trait, content in traits:
            status = "IMPLEMENTED" if trait in content else "NOT FOUND"
            print(f"  - {trait:25}: {status}")
    else:
        print("  - Fulfillment controller or model not found!")
