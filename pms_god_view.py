...
def main():
    ...
    if os.path.exists(staps_path):
        with open(staps_path, 'r') as f:
            ...
            has_error_pers = "'error': telemetry_data['error']" in content
            ...
            print(f"  - Error Detail Persistence  : {'PASSED' if has_error_pers else 'FAILED'}")
    else:
    ...
    if os.path.exists(fulfillment_path) and os.path.exists(device_path):
        with open(device_path, 'r') as f:
            content = f.read()
            traits = ['OnOff', 'Brightness', 'Scene', 'LockUnlock', 'TemperatureSetting', 'FanSpeed', 'ColorTemperature']
            for trait in traits:
        ...
        with open(fulfillment_path, 'r') as f:
            content = f.read()
            has_room = "'roomHint': dev.room_name" in content
            print(f"  - RoomHint (Standardized)   : {'PASSED' if has_room else 'FAILED'}")
    else:
        print("  - Controllers or Models not found!")
    ...
    print("\n[5] Resident Portal UI/UX (Glassmorphism 2.0):")
    ...
    if os.path.exists(css_path):
        with open(css_path, 'r') as f:
            ...
            print(f"  - Optimized Transparency      : {'ACTIVE' if has_glass else 'INACTIVE'}")
    else:
